...
The `ext:legacy_param_guard` adopts the TYPO3 Core Contribution commit message rules with minor adjustments to reflect
the use of GitHub as contribution platform.
...
1.  `Resolves:`
    You need to reference an issue on `Github Issues<https://github.com/sbuerk/t3e-legacy-param-guard/issues>`__ here simply by adding #[ISSUE_NUMBER]. For **features** and **tasks**, it closes the issue on merge.:
...
2.  `Related:`
    Other issues related to this change which are not resolved (for **all types**, it simply adds a relation, no closing). You need to reference an issue on `Github Issues<https://github.com/sbuerk/t3e-legacy-param-guard/issues>`__ by just adding the issue number like in:
...
3.  `Releases:`
    ...
    Always make sure the target version does indeed exist, when in doubt, ask in the `#typo3-cms <https://typo3.slack.com/messages/typo3-cms/>`_.
